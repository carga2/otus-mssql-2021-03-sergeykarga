...
from openrowset(bulk 'D:\КУРСЫ\MSSQL Developer\Урок8\StockItems.xml', single_clob) as xml_table
...
MERGE [Warehouse].[StockItems] AS target  -- таблица, которая будет меняться
USING (
		select *
...
ON (target.StockItemName = source.StockItemName)  -- условие связи между источником и изменяемой таблицей
WHEN MATCHED THEN 
	UPDATE 
	SET 
...
select 
	...
	,LEFT((SELECT value + ', ' as 'data()' FROM OPENJSON(CustomFields, '$.Tags') for xml path('')), LEN((SELECT value + ', ' as 'data()' FROM OPENJSON(CustomFields, '$.Tags') for xml path(''))) - 1) new_str
from Warehouse.StockItems s
where exists (SELECT value FROM OPENJSON(CustomFields, '$.Tags') where value = 'Vintage')
